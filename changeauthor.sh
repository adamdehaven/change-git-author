...
if [ "$UPDATE_REMOTE" -eq 1 ]; then
  ...
  USER_REMOTE_EXISTS="$(git ${USER_GIT_DIR} ${USER_WORK_TREE} remote show | grep -wx ${USER_REMOTE})"
...
# if [ "$SHOULD_EXECUTE" -eq 1 ] && { [ "$UPDATE_REMOTE" -eq 1 ] || [ "$REPOSITORY_VERIFIED" -eq 1 ]; }; then
if [ "$SHOULD_EXECUTE" -eq 1 ]; then
  ...
  if [ "$UPDATE_REMOTE" -eq 1 ] && [ -n "$REPOSITORY_VERIFIED" ]; then
    ...
    echo ""
    echo "${COLOR_GREEN}The commits linked to ${COLOR_GREEN}'${USER_OLD_EMAIL}' have ${COLOR_RESET}"
